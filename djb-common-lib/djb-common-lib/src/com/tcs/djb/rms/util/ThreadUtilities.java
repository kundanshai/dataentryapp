/************************************************************************************************************
 * @(#) ThreadUtilities.java 1.1 10 Dec 2014
 *  
 *
 *************************************************************************************************************/
...
/**
 * The ThreadUtilities class provides a collection of static methods for listing
 * and finding Thread, ThreadGroup, and ThreadInfo objects.
 * <p>
 * Please see the accompanying article: <a href="http://nadeausoftware.com/articles/2008/04/java_tip_how_list_and_find_threads_and_thread_groups"
 * >Java tip: How to list and find threads and thread groups</a>
 * 
...
public final class ThreadUtilities {
	...
	/**
	 * Get a list of all threads in a named thread group. A null is returned if
	 * the group cannot be found. An empty array is returned if there are no
	 * threads in the group.
	 * 
	 * @param name
	 *            the name of the thread group
	 * @return an array of threads, or null if the group is not found
	 * @throws NullPointerException
	 *             if the name is null
	 */
	...
	/**
	 * Get a list of all threads, sorted from highest to lowest priority. Since
	 * there is always at least one thread, this method never returns null or an
	 * empty array. Since threads may change their priority during this method's
	 * sort, the returned thread list may not be correct by the time it is
	 * returned.
	 * 
	 * @return an array of threads
	 */
	public static Thread[] getAllThreadsPrioritized() {
		...
		java.util.Arrays.sort(allThreads, new java.util.Comparator<Thread>() {
	...
	/**
	 * Get a list of all daemon threads. An empty array is returned if there are
	 * no daemon threads.
	 * 
	...
	public static ThreadInfo getThreadInfo(final long id) {
		...
		final ThreadInfo[] infos = thbean.getThreadInfo(new long[] { id },
				true, true);
		if (infos.length == 0)
			return null;
		return infos[0];
