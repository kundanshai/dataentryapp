 ...
 * @since 22-04-2013
 * @history 09-01-2014 Matiur Rahman Added {@link #enableObPasswordChangeFlag}
 *          to enable force password changed for password reset by a admin user.
 * @history 24-01-2014 Matiur Rahman Added feature to OAM unlock as per JTrac
 *          ID#DJB-2372.
 * 
 */
public class LDAPManager {
	...
	public static String addUser(UserDetails userDetails) {
		...
		try {
			...
			Attribute displayName = new BasicAttribute("displayName", userName
					.toUpperCase());
			Attribute givenName = new BasicAttribute("givenName", userDetails
					.getFirstName());
			/* Add Surname */
			...
			/* Add email id */
			Attribute mail = new BasicAttribute("mail", userDetails
					.getEmailId());
			/* Add mobile no */
			Attribute mobile = new BasicAttribute("mobile", userDetails
					.getMobileNo());
			...
			/* Create the entry */
			result = ldapContext.createSubcontext(getUserDN(userDetails
					.getUserId()), container);
	...
	public static boolean enableObPasswordChangeFlag(String userID) {
		...
		try {
			...
			AppLog.info("Password has been Reset by Admin User for User ID::"
					+ userID);
			/* Create Ldap Context for Administrator */
			ldapContext = getLdapContextForAdmin();
			ModificationItem[] modificationItemArray = new ModificationItem[1];
			...
			Attribute obpasswordchangeflag = new BasicAttribute(
					...
					DJBConstants.OB_PASSWORD_CHANGE_FLAG_VALUE);
	...
	public static List<UserDetails> getAllUser() throws NamingException {
		...
		try {
			...
			String filter = new StringBuffer().append("(&").append(
					"(objectClass=orclUser)").append(")").toString();
			...
			while (results.hasMore()) {
				...
				if (null != attrs.get("homePostalAddress")) {
					...
					String emailId = null != attrs.get("mail") ? attrs.get(
							"mail").get().toString() : "";
					// emailId = emailId.substring(emailId.indexOf(':') + 1)
					// .trim();
					String mobile = null != attrs.get("mobile") ? attrs.get(
							"mobile").get().toString() : "";
	...
	private static List<String> getGroups(LdapContext ctx, String userId)
		...
		// Set up criteria to search on
		String filter = new StringBuffer().append("(&").append(
				"(objectClass=orclGroup)").append("(uniqueMember=").append(
				getUserDN(userId)).append(")").append(")").toString();
	...
	private static LdapContext getLdapContextForAdmin() {
		...
		try {
			...
			env.put(Context.SECURITY_AUTHENTICATION, PropertyUtil
					.getProperty("SECURITY_AUTHENTICATION"));
			env.put(Context.SECURITY_PRINCIPAL, PropertyUtil
		...
		try {
			...
			while (results.hasMore()) {
				...
				if (null != attrs.get("homePostalAddress")) {
					...
					String emailId = null != attrs.get("mail") ? attrs.get(
							"mail").get().toString() : "";
					String mobileNo = null != attrs.get("mobile") ? attrs.get(
							"mobile").get().toString() : "";
					String isActive = (null != attrs.get("orclIsEnabled") && ("DISABLED")
							.equalsIgnoreCase(attrs.get("orclIsEnabled").get()
									.toString())) ? "NO" : "YES";
				...
				if (null != userDetails.getUserId()
					...
					List<String> groupList = getGroups(ldapContext, userDetails
							.getUserId());
	...
	private static String getUserDN(String username) {
		...
		try {
			...
			return new StringBuffer().append("uid=").append(username).append(
					",").append(USER_SEARCH_BASE).toString();
		} catch (Exception e) {
			AppLog.error(e);
		}
		AppLog.end();
	...
	public static String isLockedLDAPUser(LdapContext ldapContext, String userId)
		...
		try {
			...
			Long lockoutDuration = Long.parseLong(attrs.get(
					"pwdLockoutDuration").get().toString());
	...
	private static String removeUserFromGroup(LdapContext ctx, String userId,
		...
		try {
			...
			AppLog.info("User Id ::" + userId + " Successfully Removed from ::"
					+ groupName);
			returnMessage = "SUCCESS";
		...
		try {
			...
			if (null != ldapContext) {
				...
				if (null != newPassword && !"".equals(newPassword)
						&& !(DJBConstants.DEFAULT_PASSWORD).equals(newPassword)) {
					Attribute userpassword = new BasicAttribute("userpassword",
							newPassword);
					modificationItemArray[i++] = new ModificationItem(
	...
	public static String updateUser(UserDetails userDetails) {
		...
		try {
			...
			if (null != userDetails.getPassword()
					...
					&& !(DJBConstants.DEFAULT_PASSWORD).equals(userDetails
							.getPassword())) {
			...
			if (null != removedGroupsArray && removedGroupsArray.length > 0) {
				...
				for (int j = 0; j < removedGroupsArray.length; j++) {
					...
					if (null != groupName && !"".equalsIgnoreCase(groupName)) {
						...
						removeUserFromGroup(ldapContext, userDetails
								.getUserId(), groupName);
					}
				}
			}
